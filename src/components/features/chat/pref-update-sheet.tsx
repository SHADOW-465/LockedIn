...
export function PrefUpdateSheet({ updates, onConfirm, onDismiss }: PrefUpdateSheetProps) {
    return (
        <div className="fixed inset-0 z-[100]">
            ...
            {/* Sheet */}
            <div className="absolute bottom-0 left-0 right-0 rounded-t-2xl bg-zinc-900 border-t border-zinc-800 p-6 pb-24 sm:pb-6">
                <div className="flex items-center gap-2 mb-4">
                    <div className="w-2 h-2 rounded-full bg-[var(--accent)]" />
                    <h3 className="text-white font-semibold text-sm uppercase tracking-widest">Care Mode — Save Preference?</h3>
                </div>
                    ...
                    {updates.map((update, i) => (
                        <div key={i} className="p-3 rounded-lg bg-zinc-800 border border-zinc-700">
                            <div className="flex items-center gap-2 mb-1">
                                ...
                                <span className="text-xs px-1.5 py-0.5 rounded bg-zinc-700 text-white/80">
                ...
                <div className="flex gap-3">
                    ...
                    <button
                        onClick={onConfirm}
                        className="flex-1 py-3 rounded-xl bg-[var(--accent)] text-black text-sm font-bold hover:brightness-110 transition"
                    >
                        Save to Profile
                    </button>
                </div>
